...
def readchat(question, answer, reverse=False):
    ...
    lines = open('./static/data/Textranked.csv', encoding='utf-8').read().strip().split('\n')[1:]
...
def make_answer(question, model, tokenizer):
    q_token = get_ques_token(question)

    data = pd.read_csv('./static/data/Textranked.csv')
    ...
    if result['cosine_similarity'].iloc[0] >= 0.6:
        ...
        with torch.no_grad():
            ...
            a = ""
            while 1:
                input_ids = torch.LongTensor(tokenizer.encode(Q_TKN + question + A_TKN + a)).unsqueeze(dim=0).to(DEVICE)
